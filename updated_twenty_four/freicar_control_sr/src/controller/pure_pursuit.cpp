...
void PurePursuit::controller_step(nav_msgs::Odometry odom)
{
    ...
    try{
        ...
        if(path_.size() > 0 && index_ < path_.size()-1)
        {
//            sendGoalMsg(false);
//            completion_advertised_ = false;
            ...
            float alpha = atan2(path_[index_].getOrigin().y() - map_t_fa.getOrigin().y(), path_[index_].getOrigin().x() - map_t_fa.getOrigin().x()) - yaw;

            std::cout << "check index" << index_ << std::endl;
...
//             }
//            pub_acker_.publish(cmd_control_);
            float pos_error_x = path_[index_].getOrigin().x() - map_t_fa.getOrigin().x();
            float pos_error_y = path_[index_].getOrigin().y() - map_t_fa.getOrigin().y();
            ...
            if (fabs(pos_error_x) < 0.8 && fabs(pos_error_y) < 0.8)
            {
                ...
                mean_pos_error = sum_pos_error / index_;
...
//                std::cout << "index "<< index<< std::endl;
                ...
                {
                    std::cout << "GOAL REACHED BY PURE PURSUIT " << index_ << std::endl;
                    sendGoalMsg(true);
//                    index_ = 0;
                    ros::Duration(1).sleep();
                    sendGoalMsg(false);
