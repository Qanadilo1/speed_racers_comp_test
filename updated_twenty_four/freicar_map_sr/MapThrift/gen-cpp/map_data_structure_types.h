...
typedef struct _Lane__isset {
  _Lane__isset() : lane_markings(false), dir(false), outgoing_connections(false), incoming_connections(false), points(false), width(false), handle_point(false), visibility(false), height(false), junction_id(false) {}
  bool lane_markings :1;
  bool dir :1;
 ...
 public:
  ...
  bool operator == (const Lane & rhs) const
  {
    ...
    if (!(height == rhs.height))
      return false;
    if (!(junction_id == rhs.junction_id))
      return false;
    return true;
  }
  bool operator != (const Lane &rhs) const {
