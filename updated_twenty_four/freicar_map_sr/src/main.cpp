...
//        std::cout<<"received HIGH LEVEL COMMAND" << std::endl;

            HLC_msg.command = msg.command;
            HLC_msg.name = msg.name;
            HLC_bool = true;

            if(HLC_msg.command == "start" && HLC_msg.name == "freicar_1")
            {
                HLC_enum = 3;
            }
            else if (HLC_msg.command == "straight" && HLC_msg.name == "freicar_1")
            {
                HLC_enum = 3;
            }
            else if (HLC_msg.command == "left" && HLC_msg.name == "freicar_1")
            {
                HLC_enum = 1;
            }
            else if (HLC_msg.command == "right" && HLC_msg.name == "freicar_1")
            {
                HLC_enum = 2;
            }
            else
...
//            std::cout << "set junction_arrived" << map.GetUpcomingJunctionID(current_l_uuid) <<std::endl;
...
//            std::cout << "set junction_arrived what is it now? " << junction_arrived<< std::endl;
...
//                auto plan1 = freicar::planning::lane_follower::GetPlan(Point3D(10.866, 7.49 , 0.0), freicar::enums::STRAIGHT, 10,20);
                ...
                std::cout << "Plan here once" << std::endl;
...
//                std::cout<<"NOW STRAIGHT " << std::endl;

                /// FREICAR 2 JUNCTION
//                auto plan1 = freicar::planning::lane_follower::GetPlan(Point3D(9.2,   -1.1 , 0.0), freicar::enums::STRAIGHT, 15,25);
                // FREICAR 2 SPAWN
               // auto plan1 = freicar::planning::lane_follower::GetPlan(Point3D(-7.754, 3.207, 0.0f), freicar::enums::STRAIGHT, 15,25);
                auto plan1 = freicar::planning::lane_follower::GetPlan(Point3D(spawn_x, spawn_y, 0.0f), freicar::enums::STRAIGHT, 15,25);
...
//            std::cout<< " out junction arrived"<<junction_arrived<<std::endl;
            //std::cout<< "out HLC_bool "<<HLC_bool<<std::endl;
            //std::cout<< " out HLC_ command "<<freicar::enums::PlannerCommand{HLC_enum}<<std::endl;
...
//            std::cout<<"get current junction id" << map.GetUpcomingJunctionID(current_l_uuid) << std::endl;
//            std::cout << "old junction id" << junction_id<<std::endl;
            if (map.GetUpcomingJunctionID(current_l_uuid)!=-1  && map.GetUpcomingJunctionID(current_l_uuid) != junction_id)
            {

                tf::TransformListener enemy_listener;
                tf::StampedTransform enemy_transform;
                ...
                std::cout << "Planned once because current junction is = " <<map.GetUpcomingJunctionID(current_l_uuid) << "and old junction is =" << junction_id<< std::endl;
...
//                std::cout<< " Replan because i reached a junction and HLC"<<std::endl;
//                std::cout<< " What is the command now  HLC sent?  "<< HLC_enum<<std::endl;
...
//                current_lane = map.FindLaneByUuid(p_closest.GetLaneUuid());
                ...
                //// TEST
                std::cout<< "current HLC" << +HLC_enum <<std::endl;
...
//            else if (junction_arrived && map.GetUpcomingJunctionID(current_l_uuid) != junction_id)
//            {
//                std::cout << "ONLY ONCE ALLOWED no HLC" << std::endl;
//                junction_id = map.GetUpcomingJunctionID(current_l_uuid);
//
////                std::cout<< " junction arrived "<<junction_arrived<<std::endl;
////
////                std::cout<< " Junction reached plan again "<<std::endl;
////
////                std::cout<< " What is the command now ?  "<< HLC_enum<<std::endl;
//
//                auto plan2 = freicar::planning::lane_follower::GetPlan(Point3D(previous_lane_points.x(),previous_lane_points.y() , 0), freicar::enums::PlannerCommand{HLC_enum}, 15,25);
//                PublishPlan(plan2, 1.0, 0.1, 0.4, 300, "plan_1", tf);
//                junction_arrived = false;
...
//            }
                ...
                //// TEST
//                auto plan2 = freicar::planning::lane_follower::GetPlan(Point3D(p_closest_plan.x(),p_closest_plan.y() , 0), freicar::enums::PlannerCommand{HLC_enum}, 15,25);
//                auto plan2 = freicar::planning::lane_follower::GetPlan(Point3D(p_closest_plan.x(),p_closest_plan.y() , 0), freicar::enums::LEFT, 15,25);
...
//
//                    std::cout<< " After goal was reached"<<std::endl;
//                    std::cout<< "IS planning here wrong"<<std::endl;
//                    std::cout<< "Planning point is  x = "<< p_closest_plan.x()<< "y = " << p_closest_plan.y() <<std::endl;
...
//
//                    std::cout<< "junction arrived"<<junction_arrived<<std::endl;
//                    std::cout<< "HLC_bool "<<HLC_bool<<std::endl;
//                    std::cout<< "HLC_ command "<<freicar::enums::PlannerCommand{HLC_enum}<<std::endl;


//               std::cout<<"goal rached ?"<< goal_bool.data<< " I planned to go "<<freicar::enums::PlannerCommand{HLC_enum} << std::endl;
